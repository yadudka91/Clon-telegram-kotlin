...
import com.example.telegram2.ui.message_recycler_view.view_holders.*
...
class SingleChatAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    fun addItemToBottom(item: MessageView, onSuccess: () -> Unit) {
        if (!mListMessagesCache.contains(item)) { // содержит
            mListMessagesCache.add(item)
            notifyItemInserted(mListMessagesCache.size) // обнови останій елемент
        }
        onSuccess()
    }
